...
mod xml;
mod snapshot;
use crate::any::{decode_any, encode_any};
use crate::doc::Doc;
...
use crate::id_map::{AttrRange, ContentAttribute, IdMap};
...
fn _pycrdt(_py: Python, m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_class::<IdSet>()?;
    m.add_class::<IdMap>()?;
    m.add_class::<ContentAttribute>()?;
    m.add_class::<AttrRange>()?;
    ...
    m.add_function(wrap_pyfunction!(encode_any, m)?)?;
    m.add_function(wrap_pyfunction!(decode_any, m)?)?;
